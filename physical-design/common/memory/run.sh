...
elif  [[ "${technology}" == "tsmc40" ]]; then
  tt=tt1p1v25c
  ff=ff1p21vm40c
  ss=ss0p99v125c

  cp ${memory_path}/VERILOG/${memory_name}_${tt}.v outputs/${memory_name}.v
  cp ${memory_path}/NLDM/${memory_name}_${tt}.lib outputs/${memory_name}-typical.lib
  cp ${memory_path}/NLDM/${memory_name}_${tt}.db outputs/${memory_name}-typical.db
  cp ${memory_path}/NLDM/${memory_name}_${ff}.lib outputs/${memory_name}-bc.lib
  cp ${memory_path}/NLDM/${memory_name}_${ff}.db outputs/${memory_name}-bc.db
  cp ${memory_path}/NLDM/${memory_name}_${ss}.lib outputs/${memory_name}-wc.lib
  cp ${memory_path}/NLDM/${memory_name}_${ss}.db outputs/${memory_name}-wc.db
else
  echo "Technology not supported"
  exit 1
fi